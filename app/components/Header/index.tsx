    ...
    <header className={`fixed left-0 top-0 w-full z-99999 py-7 ${stickyMenu ? 'bg-white dark:bg-black shadow !py-4 transition duration-100' : ''}`}>
      <div className="mx-auto max-w-c-1390 px-4 md:px-8 2xl:px-0 lg:flex items-center justify-between relative">
        <div className="w-full lg:w-1/4 flex items-center justify-between">
          <a href="/">
            <Image src="/images/logo/logo-dark.svg" alt="logo" width={119.03} height={30} className="w-full hidden dark:block" />
            <Image src="/images/logo/logo-light.svg" alt="logo" width={119.03} height={30} className="w-full dark:hidden" />
          </a>

          {/* <!-- Hamburger Toggle BTN --> */}
          <button aria-label="hamburger Toggler" className="lg:hidden block" onClick={() => setNavigationOpen(!navigationOpen)}>
              ...
              {menuData.map((menuItem, key) => (
                <li key={key} className={menuItem.submenu && 'group relative'}>
                  {menuItem.submenu ? (
                    ...
                    <Link href={`${menuItem.path}`} className={pathUrl === menuItem.path ? 'hover:text-primary text-primary' : 'hover:text-primary'}>
                      {menuItem.title}
                    </Link>
                  )}
                </li>
