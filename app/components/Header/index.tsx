...
const Header = () => {
  ...
  return (
    <header
      className={`fixed left-0 top-0 w-full z-99999 py-7 ${stickyMenu ? 'bg-white dark:bg-black shadow !py-4 transition duration-100' : ''}`}>
      <div className='mx-auto max-w-c-1390 px-4 md:px-8 2xl:px-0 lg:flex items-center justify-between relative'>
        <div className='w-full lg:w-1/4 flex items-center justify-between'>
          <a href='/'>
            <Image src='/images/logo/logo-dark.svg' alt='logo' width={119.03} height={30}
              className='w-full hidden dark:block' />
            <Image src='/images/logo/logo-light.svg' alt='logo' width={119.03} height={30}
              className='w-full dark:hidden' />
          </a>

          {/* <!-- Hamburger Toggle BTN --> */}
          <button aria-label='hamburger Toggler' className='lg:hidden block'
        ...
        <div
          className={`w-full lg:w-full h-0 lg:h-auto invisible lg:visible lg:flex items-center justify-between ${
            navigationOpen && '!visible bg-white dark:bg-blacksection shadow-solid-5 h-auto max-h-[400px] overflow-y-scroll rounded-md mt-4 p-7.5'
          }`}
        >
          <nav>
          ...
          <div className='flex items-center gap-6 mt-7 lg:mt-0'>
            ...
            {/* <Link href="https://github.com/NextJSTemplates/solid-nextjs" className="text-waterloo text-regular font-medium hover:text-primary">
             GitHub Repo 🌟
             </Link> */}

            <div
              className='flex cursor-pointer items-center justify-center bg-primary hover:bg-primaryho ease-in-out duration-300 text-white text-regular rounded-full py-2.5 px-7.5'
            >
              Login
            </div>
          </div>
